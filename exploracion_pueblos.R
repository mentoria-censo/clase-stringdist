...
pueblos = arrow::read_feather('data_pueblos_codificacion.feather' %>% glue, as_data_frame = T)
...
afro_o_mapuche = pueblos_otro %>% count(estr_p16glospueblo ) %>% filter(str_detect(estr_p16glospueblo, 'MAPU|(MEÑ.$)'))
afro_o_mapuche = pueblos_otro %>% count(estr_p16glospueblo ) %>% filter(str_detect(estr_p16glospueblo, 'TOD.S'))

expand.grid(afro_o_mapuche$estr_p16glospueblo, c('ATACAMEÑO', 'MAPUCHE')) %>%
